...
const Funding = () => {
  ...
  const fund = async () => {
    ...
    if (!amount || !accountName || !pin) {
      ...
      let eachDetail = { amount, accountName, cTime };
      await setalldetails(() => {
  ...
  return (
    <>
      <Navbar fundcolor="text-white" />

      <center>
        <p className={styles.p}>Fund Account</p>
        <div>
          ...
          <br /> <br />
          <button className={styles.button} onClick={fund}>
            Fund Account
          </button>
        </div>
      </center>
