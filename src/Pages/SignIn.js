...
const SignIn = ({ allUsers }) => {
  ...
  const signin = () => {
    ...
    for (let i = 0; i < allUsers.length; i++) {
      if (
        logemail == allUsers[i].email &&
        logpassword == allUsers[i].password
      ) {
    ...
    <>
      <div>
        ...
        >
          ...
          <br /> <br />
          {error ? (
            <p className="bg-danger">Invalid email or password input</p>
          ) : (
            ""
          )}
          ...
          <div className={styles.center}>
            <Link to="/signup" className={styles.whitecolor}>
