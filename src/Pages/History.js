      ...
      <div>
        <center>
          <div>
            ...
            {debit.length == 0 ? (
              <div style={styleNoReg}> No Debit Transactions</div>
            ) : (
              ""
            )}
            {debit.map((item, index) => (
              <div key={index} className={styles.n}>
                Bank Name: {item.bankName}. <br /> Account Number:{" "}
                {item.accountNumber}. <br />
                Beneficiary Name: {item.accountName}. <br />
                {item.cTime}{" "}
                <span className={styles.debit}> - #{item.amount} </span>
              </div>
            ))}
          </div>{" "}
          <br />
          <div>
            ...
            {credit.length == 0 ? (
              <div style={styleNoReg}> No Credit Transactions</div>
            ) : (
              ""
            )}
            {credit.map((item, index) => (
              <div key={index} className={styles.n}>
                Source: {item.accountName}. <br />
                {item.cTime}{" "}
                <span className={styles.credit}>+ #{item.amount}</span>
              </div>
            ))}
          </div>{" "}
          <br />
